...
while [ $httpstatus -ne "200" ]; do
  httpstatus=$(curl -s -o /dev/null -L -w "%{http_code}" http://etcd:2379/version)
done
...
if [ keyexists != 200 ]
  then
    ...
    curl http://etcd:2379/v2/keys/squirrel/master/mobility_manager -XPUT -d value="InteractivePositions"
    ...
    curl http://etcd:2379/v2/keys/squirrel/master/mobility_manager_config_path -XPUT -d value="/squirrel/master/InteractivePositions"
    curl http://etcd:2379/v2/keys/squirrel/master/InteractivePositions/laddr -XPUT -d value=":9999"
